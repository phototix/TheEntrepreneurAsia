...
$filterSQL="";
if($filter<>""){
  $filterSQL=" AND page_slug='$filter'";
}
?>
...
<div class="form-group">
  <div id="control-demo-6" class="col-sm-4">
    <select class="form-control" name="filter" onchange="form.submit()">
      <option value="">Select a page to filter images/show all</option>
      <?php
      $result=mysqli_query($con, "SELECT DISTINCT page_slug FROM db_gallery WHERE image_stat<>'3'");
      while($row=mysqli_fetch_array($result)){
      ?>
      <option value="<?=$row["page_slug"]?>"<?php if($filter==$row["page_slug"]){ ?> selected="selected"<?php } ?>><?=ucfirst(str_replace("-", " ", $row["page_slug"]))?></option>
      <?php } ?>
    </select>
  </div>
</div><!-- .form-group -->
...
<div class="table-responsive">
	...
	<table id="default-datatable" data-plugin="DataTable" class="table table-striped" cellspacing="0" width="100%">
		...
		<tbody>
			<?php
			$strNumber="";
			$result=getTable("db_gallery", "WHERE image_stat<>'3'".$filterSQL." ORDER BY image_id DESC");
          ...
          <input type="text" id="url-<?=$row["image_id"]?>" value="<?=$row["image_url"]?>" style="width:100%;border:0px;">    
            ...
            <button type="button" class="btn btn-info btn-xs" onclick="copyToClipboard(document.getElementById('url-'+<?=$row["image_id"]?>))"><i class="fa fa-copy"></i></button>
            <?php if($row["page_slug"]=="general"){ ?>
...
<input type="hidden" name="form" value="pages-image-library-deleteImage">
        ...
        <div class="widget-body">
          <div class="row">
            <form action="" id="uploadImageGallery" class="form-horizontal" method="post" enctype="multipart/form-data">
              ...
              <input type="hidden" name="form" value="pages-image-library-uploadImageGallery">
