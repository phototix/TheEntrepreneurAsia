...
<a href="?page=<?=$page?>&cate=<?=$cate?>&action=extraprojectInfo&id=<?=$id?>">
	<button type="button" class="btn btn-default" ><i class="fa fa-pencil"></i> Project Info</button>
</a>
<a href="?page=<?=$page?>&cate=<?=$cate?>&action=extragoogleForm&id=<?=$id?>">
	<button type="button" class="btn btn-success" ><i class="fa fa-pencil"></i> Google Form</button>
</a>
<br><br>
...
<div class="widget-body">
	...
	<form action="#" class="form-horizontal" method="post" data-parsley-validate>
		...
		<textarea class="input-block-level" id="FCKeditor1" name="txtContent" rows="18"><?=getDataTable("db_blog_post", "post_id", $id, "extraprojectInfo")?></textarea>
