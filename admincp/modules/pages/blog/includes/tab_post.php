...
<div class="row">
  <div class="col-sm-3">
    <a href="?page=<?=$page?>&cate=<?=$cate?>&action=addBlogPost">
    	<button type="button" class="btn btn-success" ><i class="fa fa-pencil"></i> Write a New Post</button>
    </a>
  </div>

  <?php $result=mysqli_query($con, "SELECT DISTINCT cate_id FROM db_blog_post WHERE post_stat<>'3'"); ?>
  <?php
...
<div class="table-responsive">
	...
	<table id="default-datatable" data-plugin="DataTable" class="table table-striped" cellspacing="0" width="100%">
		...
		<tbody>
			...
			$result=mysqli_query($con, "SELECT * FROM db_blog_post WHERE post_stat<>'3'".$cate_idSQL." ORDER BY post_id DESC");
			while($row=mysqli_fetch_array($result)){
				$strNumber++;
        $resultComment=getTable("db_blog_post_comments", "WHERE comment_stat<>'3' AND comment_display=1 AND blog_id='".$row["post_id"]."'");
        ...
        <td>
          ...
          <?php $useThis="yes";if($useThis=="yes"){?>
            <a href="/post/<?=$row["post_token"]?>?title=<?=$row["post_slug"]?>&function=preview" target="_blank">
...
<input type="hidden" name="form" value="pages-blog-deletePost">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Update Blog Status #<span><code id="postUpdate-Title">title</code></span></h4>
      </div>
        ...
        <div class="widget-body">
          <div class="row">
            <form action="" class="form-horizontal" method="post">
              ...
              <input type="hidden" name="form" value="pages-blog-updatePostStatus">
              <input type="hidden" id="txtID" name="txtID">
              <select class="form-control" id="txtStatus" name="txtStatus">
                <option value="1">Publish</option>
                <option value="1-2">Publish as feature</option>
...
<div class="modal fade" id="postPreview" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Blog Preview #<span><code id="postPreview-Title">title</code></span></h4>
      </div>
...
function showPostComments(postID, postTitle){
  ...
  showPostCommentsAjax.open("GET","../modules/pages/blog/loadPostComments.php?id=" + postID ,true);
...
function deleteComment(postID, commentID, postTitle){
  swal({
    ...
    deleteCommentAjax.open("GET","../modules/pages/blog/deletePostComments.php?id=" + commentID ,true);
        ...
        <div class="widget-body">
          <div class="row">
            <form action="" id="replyCommentForm" class="form-horizontal" method="post">
              ...
              <input type="hidden" value="pages-blog-replyPostComment" name="form">
