<?php
$txtPostType=getDataTable("db_annoucement_post", "post_id", $id, "cate_id");
?>
<a href="?page=<?=$page?>&cate=<?=$cate?>">
	<button type="button" class="btn btn-default" ><i class="fa fa-arrow-left"></i> Back to Announcement Post</button>
</a>
...
<header class="widget-header">
	...
	<h4 class="widget-title">Edit Announcement Post <?=getDataTable("db_annoucement_post", "post_id", $id, "post_title")?></h4>
...
<div class="widget-body">
	...
	<form action="#" class="form-horizontal" method="post" data-parsley-validate>
		...
		<input type="hidden" name="form" value="pages-announcement-editPost">
		
		<div class="form-group">
			<label for="control-demo-1" class="col-sm-2">Title</label>
			<div class="col-sm-10">
				<input type="text" name="txtTitle" id="txtTitle" class="form-control" value="<?=getDataTable("db_annoucement_post", "post_id", $id, "post_title")?>" onkeyup="convertToSlug(this.value)" onchange="convertToSlug(this.value)">
			</div>
		</div><!-- .form-group -->
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<input type="text" name="txtImage" placeholder="Image URL here..." class="form-control" value="<?=getDataTable("db_annoucement_post", "post_id", $id, "post_cover")?>">
			</div>
		</div><!-- .form-group -->
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<input type="text" name="txtURLSlug" id="txtURLSlug" class="form-control" required="required" value="<?=getDataTable("db_annoucement_post", "post_id", $id, "post_slug")?>" readonly="readonly">
			</div>
		</div><!-- .form-group -->

		<?php
		$result=getTable("db_annoucement_post_category", "WHERE cate_stat<>'3'");
		if(count($result)>0){
		?>
		<div class="form-group">
		...
		<textarea class="input-block-level" id="FCKeditor1" name="txtContent" rows="18"><?=getDataTable("db_annoucement_post", "post_id", $id, "post_contents")?></textarea>
